...
module pisces_optics
   ...
   type, extends(type_base_model), public :: type_pisces_optics
      type (type_state_variable_id)     :: id_ch   ! Mokrane
      ...
      type (type_surface_diagnostic_variable_id) :: id_heup, id_heup_01, id_emoy, id_zpar , id_zqsr , id_dqsr, id_dpqsr100
...
contains
   ...
   subroutine initialize(self, configunit)
      ...
      !class (type_dia),                      pointer :: dia_model  ! Mokrane
      ...
      !call self%add_child(dia_model, 'dia')
      !call dia_model%request_coupling(dia_model%id_ch, '../ch')
   ...
   subroutine do_column(self, _ARGUMENTS_DO_COLUMN_)
      ...
      ELSE
         ...
         zqsr_mean = self%xparsw * qsr_mean  !Mokrane replace qsr_mean by qsr
      ENDIF
      ...
      _DOWNWARD_LOOP_BEGIN_
         ...
         !ekb = ekb + rkrgb(1,irgb) * e3t_n ! commented by Mokrane
         !ekg = ekg + rkrgb(2,irgb) * e3t_n ! commented by Mokrane
         !ekr = ekr + rkrgb(3,irgb) * e3t_n ! commented by Mokrane
         ...
         f1 = EXP(-ekb -0.5_rk * rkrgb(1,irgb) * e3t_n ) !EXP( -0.5_rk * ekb ) !Mokrane
         f2 = EXP(-ekg -0.5_rk * rkrgb(2,irgb) * e3t_n ) !EXP( -0.5_rk * ekg ) !Mokrane
         f3 = EXP(-ekr -0.5_rk * rkrgb(3,irgb) * e3t_n )!EXP( -0.5_rk * ekr ) !Mokrane
         pe1 = zqsr_mean * f1   
         pe2 = zqsr_mean * f2  
         pe3 = zqsr_mean * f3   
         etot = zqsr * (f1 + f2 + f3)
