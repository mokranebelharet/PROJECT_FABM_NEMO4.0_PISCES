...
module pisces_turbocline
   ...
   type, extends(type_base_model), public :: type_pisces_turbocline
      ...
      type (type_diagnostic_variable_id) :: id_davt ! Mokrane : to save and output avt variable
...
contains
   ...
   subroutine initialize(self, configunit)
      ...
      call self%register_diagnostic_variable(self%id_davt, 'davt', 'm2s-1','diagnostic avt', source=source_do_column)  ! Mokrane
...
!-------------- Mokrane ---------------------------
        _GET_(self%id_avt, avt)
       _SET_DIAGNOSTIC_(self%id_davt, avt)
   end subroutine
